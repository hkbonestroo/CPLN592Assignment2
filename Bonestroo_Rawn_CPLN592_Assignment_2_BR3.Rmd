...
***
# 1.1 Introduction
Our team was tasked with building a better predictive model of home prices for Miami, Florida (Miami Beach was also included, but for brevity the combined municipalities will be referred to as Miami) utilizing online open data resources and OLS regression. We attempted to minimize average error and create a model that is both accurate and generalizable across space. While we initially collected many variables, we found that a simple model including only four features was best able to predict sale prices. This is because our models with many features included colinear features, and thus, were overfitted. By including neighborhood and the average sale price of surrounding homes, we were able to better predict sale prices across neighborhoods and, importantly, within neighborhoods. The following map depicts the the distribution of sale prices of the provided homes across Miami.

# Objective:
To build a model that predicts home values in Miami Florida as accurately as possible, utilizing regression and available public data. 
...
```{r Miami Sale Price distribution, results='hide', error=FALSE, message=FALSE, warning=FALSE,fig.asp=1}


# Plot distribution of sale prices over neighborhoods
ggplot() +
  geom_sf(data = nhoods, fill = "grey40") +
  geom_sf(data = Miami_Training, aes(colour = q5(SalePrice)), 
...
The feature with strongest statistically significant correlation to sale price was the actual square foot of the home with a correlation coefficient of .88. Other features with strong correlations to sale price include SalePriceAvg (r=.76),LotSize (r=.71), and lagSQ (r=.65). Some of the next strongest positive correlations include daycare_nn2 (r=.45) and MedHHInc (r=.41). These relationships can be explored through the following scatterplots. 
...
st_drop_geometry(Miami_Training) %>% 
  ...
  gather(Variable, Value, -SalePrice) %>% 
   ggplot(aes(Value, SalePrice)) +
     ...
     labs(title = "Price as a function of continuous variables", caption= "Figure 1.2.2") +
     plotTheme()

```
These plots suggest a positive correlation exists. In all four plot, the regression line slopes upward, showing that as the variables increase, so does sale price. Many of our features we selected were not numerical, so they cannot be depicted with scatter plots. Instead, we examined the relationship between these features and the sale price using bar charts.
...
st_drop_geometry(Miami_TrainingGeom) %>% 
  ...
  gather(Variable, Value, -SalePrice) %>% 
   ggplot(aes(Value, SalePrice)) +
     ...
     labs(title = "Price as a function of categorical variables", y = "Mean_Price",caption="Figure 1.2.3") +
     theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
     plotTheme()

```
Maps can also be used to examine the relationship between features and sale price. By comparing maps with each other interesting patterns that inform the creation of the model can appear. 

```{r Miami Maps, results='hide', error=FALSE, message=FALSE, warning=FALSE,fig.asp=1}


# Plot zoning
ggplot() +
  geom_sf(data = nhoods, fill = "grey40") +
  geom_sf(data = Miami_TrainingGeom, aes(colour = Zoning), 
          show.legend = "point", size = .75) +
...
ggplot() +
  geom_sf(data = nhoods, fill = "grey40") +
  geom_sf(data = Miami_TrainingGeom, aes(colour = Dock), 
          show.legend = "point", size = .75) +
...
ggplot() +
  geom_sf(data = nhoods, fill = "grey40") +
  geom_sf(data = Miami_TrainingGeom, aes(colour = q5(daycare_nn2)), 
          show.legend = "point", size = .75) +
  scale_colour_manual(values = palette5,
...
ggplot() +
  geom_sf(data = nhoods, fill = "grey40") +
  geom_sf(data = Miami_TrainingGeom, aes(colour = q5(SalePriceAvg)), 
          show.legend = "point", size = .75) +
  scale_colour_manual(values = palette5,
...
Figure 1.2.5 shows that not surprisingly, homes with docks are clustered near the shore and are especially prevalent in Miami Beach. Figures 1.2.4 and Figures 1.2.6 show an interesting pattern between average distance to the nearest two daycare centers and zoning becomes apparent. It looks like daycare centers are less prevalent in areas that are in the zoning class 0100- single family- general. For the most zoning tends to change along neighborhood lines. 

# 1.3 Methods

We created the desired sale price prediction model using the following steps. First, we examined the correlation between each feature and sale price, looking for those features with the strongest relationships. We also examined the correlations between each feature to look for collinearity. Using this information, we narrowed down our features to exclude features that were strongly correlated to each other and those with a weak correlation to sale price. We then found the R^2 and MAE for the model which includes all of our variables as a baseline. R^2 is the proportion of variance in sale price which can be explained by our features. While R^2 shows how well our model fits the data we have, a high R^2 value doesn't mean the model is good at predicting prices of unseen data. MAE or mean absolute error is a better measure of how good a model is good at prediction. The MAE is found by splitting the house points we have into a training and test set. Using the model created with the training set we predicted sale prices for the test set. These are then compared to the actual known sale prices of the test set to determine the error. The MAE is average of the absolute error for each of the predicted sale prices. We created a function which determined every single combination of features which could go into a model. We then had a function test every combination to determine which combinations of features produced the lowest MAE. We then used cross-validation to test the models across different random samples of data. We also examined if our model was good at predicting across neighborhoods and within neighborhoods and checked to see if our larger absolute errors were clustered.

# 1.4 Results
## Ordinary Least Squares Regression (OLS)
We evaluated the predictive power of different combinations of variables by dividing the dataset into a "Training" set and a "Test" set that allow us to test how our model might perform on a real dataset. 60% of houses with known sale values were grouped randomly into the Training Set, and 40% was grouped into the Test Set. A regression model was then created with the Training Set using our final variable selection of Neighborhood, nearby Sale Price Average, Zoning, and presence of a Dock. The output of this regression model when run on the Training Set is summarized below.

``````{r OLS, results='asis',error=FALSE, message=FALSE, warning=FALSE,fig.width=24, fig.height=8}
...
kable(reg.summary,format = "html", caption = "Title of the table") %>% 
  kable_styling()
...
```
## MAE and MAPE (Single Test)
Using the same regression variables described above, we applied our model to the houses in our Test Set in order to create predicted sale prices. In order to evaluate the effectiveness of the model, we started by calculating the average difference between the predicted prices and actual prices (Sale Price Error), the absolute value of those differences (Sale Price Absolute Error), and the percentage error of the absolute differences (Average Percentage Error).

calculated Test Set was given several new columns which represent the predicted sale price using the regression model created previously. The output tells us the mean absolute error (MAE) and mean absolute percentage error (MAPE) of the Test Set.
``````{r MAE and MAPE, results='asis',error=FALSE, message=FALSE, warning=FALSE,fig.width=24, fig.height=2}
...
Single_test = array(c(AbsError,APE), dim=c(1,2))

kable(Single_test,format = "html", caption = "Title of the table",col.names = c('Mean Absolute Error', 'Mean Absolute Percent Error')) %>% 
  kable_styling() %>%
  footnote(general_title = "\n",
           general = "Table 1.4.3")
...
The Test Set was further analyzed through a K-Fold Cross Validation technique. Instead of calculating the MAE and MAPE based on the results of one single randomly selected Test Set, this approach allows the process to be repeated 100 times and then averaged together to achieve a higher degree of accuracy. The results can be seen in the histogram below.
...
kable(kfold_results,format = "html", caption = "Title of the table", col.names = c('', '')) %>% 
  kable_styling() %>%
  footnote(general_title = "\n",
           general = "Table 1.4.3")
...
ggplot(data=MAEs, aes(MAEs$MAE)) + 
  ...
  labs(title="Histogram for Age", x="MAEs", y="Count",  subtitle="Miami, Florida", 
       caption= "Figure 1.1.1")+
  plotTheme()
...
In order to further analyze the question of whether errors in our model were spatially clustered, we looked at the Spatial Lag of each house. 
...
```{r Spatial Lag2,echo=FALSE, error=FALSE, message=FALSE, warning=FALSE,fig.asp=.5}
#Calculate and plot spatial lag of the training set
Miami_Training.centroids<-st_centroid(Miami_Training)
coords <- st_centroid(st_geometry(Miami_Training), of_largest_polygon=TRUE)
neighborList <- knn2nb(knearneigh(coords, 5))
...
ggplot(data=Miami_test_plot, aes(lagPriceError,SalePrice.Error)) +
  geom_point()+
  geom_smooth(method=lm, se=FALSE)+
  labs(title="Lag Price Error vs. Sale Price Error", x="MAEs", y="Count",  subtitle="Miami, Florida", 
       caption= "Figure 1.1.1")+
  plotTheme()
...
The graphic below plots our price predictions on the Test Set next to price predictions on the Training Set. The output appears to be consistent across both, which is unsurprising given the fact that the Test Set was selected randomly from the same group of houses ast the Training Set. 

```{r Predicted Maps,echo=FALSE, error=FALSE, message=FALSE, warning=FALSE,fig.asp=.5}

# predicting Challenge set
Miami_Houses <- 
  rbind(
    st_read("studentsData.geojson") %>%
      st_transform(st_crs('EPSG:6346')))
...
ggplot() +
  geom_sf(data = nhoods, fill = "grey40") +
  geom_sf(data =MAEPrediction, aes(colour = q5(SalePrice.Predict)), 
          show.legend = "point", size = .75) +
  scale_colour_manual(values = palette5,
...
ggplot(data=MAPE.Summary, aes(mean.Price,mean.MAPE)) +
  geom_point()+
  geom_smooth(method=lm, se=FALSE)+
  labs(title="Histogram for Age", x="Mean Price", y="MAPE",  subtitle="Miami, Florida", 
       caption= "Figure 1.1.1")+
  plotTheme()
...
Spatial Error
Our Moran's I test reveals that there is a non-trivial amount of spatial autocorrelation of our model's errors that cannot be explained by random chance. Although completely eliminating spatial error may be challenging, a value of .15 is not ideal. We would be interested in seeing whether this spatial error would be resolved by addressing some of the more glaring problems relating to variable selection. 
