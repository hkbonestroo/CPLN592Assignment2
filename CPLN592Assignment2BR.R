...
Miami_Houses <- Miami_Houses %>%
  mutate(lat = unlist(map(Miami_Houses.centroids$geometry,1)),
         long = unlist(map(Miami_Houses.centroids$geometry,2)))
...
#Create WalkScore
#Example: getWS(-80.194688,25.777070,"a0a34de0dd2261f99677763bb3861e33")

WalkScore = function(x, output) {
  long = x[119]
  lat = x[120]
  getWS(long, lat, "a0a34de0dd2261f99677763bb3861e33")
}
  apply(Miami_Houses, 1, WalkScore)

for(row in 1:nrow(Miami_Houses)){
Miami_Houses <- Miami_Houses %>%
  mutate(WalkScore <- getWS(Miami_Houses$long,Miami_Houses$lat,"a0a34de0dd2261f99677763bb3861e33"))
}

#Create TransitScore
#Example: getTS(-80.211933,25.796983, "Miami", "FL","a0a34de0dd2261f99677763bb3861e33")
...
miami_metros.sf <- st_as_sf(miami_metros, coords = c("LAT", "LON"), crs = 4326, agr = "constant") %>% 
  st_transform(st_crs(Miami_Houses))
...
miami_municipalparks <- 
  rbind(
    st_read("https://opendata.arcgis.com/datasets/a585b193a4764760802f510f8c5b1452_0.geojson") %>%
      st_transform('EPSG:6346'))
miami_municipalparks <- st_set_crs(nhoodsmiami, 6346)

Miami_Houses <-
  Miami_Houses %>% 
  mutate(
    ParksDist = st_distance(Miami_Houses.centroids, miami_municipalparks))

#nearest neighbor parks (method 2 - NOT WORKING)
miamiparks <- 
  rbind(
    st_read("MunicipalParks/MunicipalParks.shp") %>% 
      st_transform(st_crs('EPSG:6346')))
...
Miami_Houses <-
  Miami_Houses %>% 
  mutate(
    Parks_nn1 = nn_function(st_c(st_centroid(Miami_Houses)), st_c(st_centroid(miamiparks)), 1))
...
miamigolfcourses <- 
  rbind(
    st_read("https://opendata.arcgis.com/datasets/229eeac512b043f8bf5317ec8377f151_0.geojson") %>%
      st_transform('EPSG:6346'))
miamigolfcourses <- st_set_crs(nhoodsmiami, 6346)

Miami_Houses <-
  Miami_Houses %>% 
  mutate(
    GolfCourseDist = st_distance(Miami_Houses.centroids, miamigolfcourses))
