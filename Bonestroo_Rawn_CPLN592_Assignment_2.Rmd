...
Plotting the prices predicted by the model against actual observed prices allows us to diagnose where the model seems to be predicting accurately and where it is not. Our predicted line (Green) starts above the perfect prediction line (Orange), suggesting that we are over-predicting home values for very low priced homes. However, this relationship soon reverses and the graph reveals that our model is under-predicting the price of homes as the true sale price increases. 
...
Next we attempted to answer the question of whether errors in our model were spatially clustered. To achieve this, we looked at the Spatial Lag of each house. 
...
We also examined spatial autocorrelation by looking at Moran's I, which is represented by the orange line on the graph. The fact that this line is located at .15 implies that there does appear to be some spatial autocorrelation to the clustering of our errors than would be explained by random chance. 
...
In one neighborhood most likely due to the under prediction of very expensive houses.
