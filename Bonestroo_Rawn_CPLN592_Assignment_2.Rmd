                                     ...
                                     ifelse(grepl("Bird Grove West", Miami_Houses$neighborhood), Miami_Houses$neighborhood2<-"Bird Grove East",
                                            ifelse(grepl("Bay Heights", Miami_Houses$neighborhood), Miami_Houses$neighborhood2<-"North Grove",
                                                   ifelse(grepl("Vizcaya", Miami_Houses$neighborhood), Miami_Houses$neighborhood2<-"North Grove",
                                                          ifelse(grepl("Fair Isle", Miami_Houses$neighborhood), Miami_Houses$neighborhood2<-"North Grove",
                                                                 Miami_Houses$neighborhood2<-Miami_Houses$neighborhood)))))
...
st_drop_geometry(Miami_TrainingGeom) %>% 
  dplyr::select(SalePrice, Zoning, midschool, Dock, neighborhood) %>%
  filter(SalePrice <= 1000000) %>%
  gather(Variable, Value, -SalePrice) %>% 
   ggplot(aes(Value, SalePrice)) +
...
reg.training <- lm(SalePrice ~ ., data = st_drop_geometry(Miami_TrainingGeom) %>% 
...
stargazer(reg.training, type="html", out="models.htm", report=('vc*p'),
          header = FALSE,
  title = "OLS Summary Results",
  notes = "Table 1.4.1 ",
                notes.align = "l",
                float = TRUE,
                table.placement = "H")
...
#Create table of regression  output
stargazer(reg.training2, type="html", out="models.htm", report=('vc*p'),
          header = FALSE,
  title = "OLS Summary Results of Model Used",
  notes = "Table 1.4.2 ",
                notes.align = "l",
                float = TRUE,
                table.placement = "H")
...
Miami_test_new <-
  Miami_test_new %>%
  mutate(SalePrice.Predict = predict(reg.training2, Miami_test_new),
...
``````{r cross-validation, results='asis',error=FALSE, message=FALSE, warning=FALSE,fig.width=12, fig.height=2}
...
MAE <-mean(reg.cv$resample[,3])
R2<-mean(reg.cv$resample[,2])
RMSE <-mean(reg.cv$resample[,1])
Kfoldresults <- array(c(RMSE,R2,MAE), dim=c(1,2,3))
kable(Kfoldresults,format = "html", caption = "Title of the table",col.names = c('Root Mean Square Error', 'R^2', 'Mean Absolute Error')) %>% 
  kable_styling() %>%
  footnote(general_title = "\n",
           general = "Table 1.4.3")
reg.cv
