...
***
# 1.1 Introduction
In order to better factor in local intelligence, our team was tasked with building a better predictive model of home prices for Miami, Florida. Using data gathered from online open data resources and OLS regression, we created a model that predicted sale prices. We attempted to minimize average error and create a model that is both accurate and generalizable across space. While we initially collected many variables, we found that a simple model including only four features was better able to predict sale prices. This is because our models with many features included colinear features, and thus, was overfitted. By including neighborhood and the average sale price of surrounding homes, we were able to better predict sale prices across neighborhoods and within neighborhoods. 
