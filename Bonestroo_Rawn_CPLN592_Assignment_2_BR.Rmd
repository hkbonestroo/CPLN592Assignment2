...
inTrain <- createDataPartition(
    y = paste(Miami_Training$neighborhood, Miami_Training$Zoning), 
    p = .60, list = FALSE)
...
stargazer(reg.training, type="html", out="models.htm")

```

```{r Create single test set, results='hide', error=FALSE, message=FALSE, warning=FALSE,fig.asp=0.5}
# single test set
Miami_test_new <-
  Miami_test_new %>%
...
knitr::kable(Single_test,  col.names = c('AbsError', 'APE'), caption = "Single Test Set", digits = c(1,3))
```
...
```
```{r spatial lag, results='hide', error=FALSE, message=FALSE, warning=FALSE,fig.asp=0.5}
#Spatial Lag (4.2)
...
```{r Moran's I,results='hide', error=FALSE, message=FALSE, warning=FALSE,fig.asp=0.5}
...
Miami_Houses$Bed <-is.factor(Miami_Houses$Bed)
Miami_Houses$Bath <-is.factor(Miami_Houses$Bath)
Miami_Houses$Stories<-is.factor(Miami_Houses$Stories)
