...
#Miami_Houses$nhoods2 <- if(Miami_houses$nhoods == "")
...
inTrain <- createDataPartition(
    y = paste(Miami_Houses$neighborhood, Miami_Houses$Zoning), 
    p = .60, list = FALSE)
...
reg.training <- lm(SalePrice ~ ., data = st_drop_geometry(Miami_training_new) %>% 
                     dplyr::select(SalePrice,lagSQ,Dock,Whirlpool,Carport,
...
Miami_test_new[is.na(Miami_test_new)] = 0
